...
generateRadioRow <- function(rowId, minLabel, choiceNames, choiceValues,
...
generateRadioMatrix <- function (inputId, rowIds, minLabels,
  ...
  rows <- lapply(1:length(rowIds), function(i){
    generateRadioRow(rowId = rowIds[[i]], minLabel = minLabels[[i]],
...
validateParams <- function(rowIds, minLabels, selected, choiceNames, labelsWidth){

  if (is.null(selected)){
    checks <- list(rowIds, minLabels)
  } else {
    checks <- list(rowIds, minLabels, selected)
  }

  lengths <- sapply(checks, length)

  if (length(unique(lengths)) > 1) {
    stop("All of rowIds, minLabels, selected should be of the same length!")
  }
...
radioMatrixInputJ <- function(inputId, rowIds, minLabels, choices = NULL,
  ...
  validateParams(rowIds, minLabels, selected, args$choiceNames,labelsWidth)
